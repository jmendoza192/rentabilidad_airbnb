...
# =========================================================
# 0. CONFIGURACIÓN Y ESTÉTICA DE SISTEMA (SANS-SERIF PURO)
# =========================================================
try:
...
# Definición de fuentes de sistema (No Roboto/WebFonts)
...
if check_password():
    # CSS: ELIMINACIÓN DE +/- Y UNIFICACIÓN DE FUENTES SANS-SERIF
    st.markdown(f"""
        <style>
        /* Reset de Fuente Global a Sans-Serif de Sistema */
        ...
        /* Limpieza de Inputs (No botones + o -) */
        ...
        /* Tarjetas Industriales (Letras Blancas) */
        .card-base {{
            border: 1px solid #30363d; border-radius: 6px; padding: 18px;
        ...
        .footer-tip {{
            background-color: #0d1117; color: #ffffff; padding: 22px;
            border: 1px solid #30363d; border-left: 4px solid #58a6ff; margin-top: 35px;
        }}
        </style>
        """, unsafe_allow_html=True)

    with st.sidebar:
        st.header("Parámetros de entrada ⚙️")
        val_depa = st.number_input("Precio de venta (S/.) 🏠", value=450000)
        inv_amoblado = st.number_input("Inversión amoblado (S/.) 🛋️", value=50000)
        st.write("---")
        tcea = st.number_input("Tcea bancaria (%) 🏦", value=9.8)
        plazo_años = st.selectbox("Plazo del crédito (Años) 🗓️", [10, 15, 20, 25], index=2)
        st.write("---")
        tarifa_base = st.number_input("Tarifa por noche (S/.) 💰", value=350)
        ocupacion_act = st.selectbox("Días ocupados al mes 🌙", list(range(1, 31)), index=19)
        st.write("---")
        renta_trad = st.number_input("Renta tradicional (S/.) 🏠", value=3000)
    ...
    with tabs[0]:
        ...
        st.markdown('<p class="info-box">Cálculo del capital propio y el apalancamiento financiero para la adquisición del activo 🏢.</p>', unsafe_allow_html=True)
        ...
        st.markdown('<p class="info-box">Determinación de la liquidez mensual neta después de obligaciones bancarias y tributarias 🧾.</p>', unsafe_allow_html=True)
        ...
        st.markdown('<div class="section-title">Sección de rentabilidad comparada 📈</div>', unsafe_allow_html=True)
        ...
        with cr2:
            st.table(pd.DataFrame({
                "Alternativa de inversión": ["ROI AIRBNB", "S&P 500 (USA)", "Factoring Local", "Fondos Mutuos", "Depósito Plazo Fijo", "Bonos Soberanos"],
                "Rendimiento anual": [f"{roi_airbnb:.1f}%", "10.0%", "12.0%", "8.5%", "7.2%", "5.8%"]
            }))
        ...
        with cp1:
            ...
            st.markdown('<p class="info-box">Este indicador define el año en que el flujo de caja neto iguala la inversión de capital propio inicial 🏛️.</p>', unsafe_allow_html=True)
        with cp2:
            fig_pb = go.Figure()
            ...
            fig_pb.update_layout(title="Curva de balance de capital (Balance neto)", height=350, paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', font=dict(family=SYS_SANS, color="white"))
            st.plotly_chart(fig_pb, use_container_width=True)

        st.markdown('<div class="footer-tip">💡 <b>Recomendación operativa:</b> Un flujo de caja positivo desde el mes 1 asegura que el activo se pague solo, permitiendo reinvertir el excedente 🚀.</div>', unsafe_allow_html=True)

    # --------------------------------------------------------- PESTAÑA 2
    with tabs[1]:
        st.markdown('<div class="section-title">Evolución del patrimonio y plusvalía 🏔️</div>', unsafe_allow_html=True)
        st.markdown('<p class="info-box">Estimación de la ganancia de capital basada en la revalorización del entorno urbano y comercial 📈.</p>', unsafe_allow_html=True)
        p_slider = st.slider("Porcentaje de plusvalía anual estimada (%)", 0.0, 10.0, 4.0)
        ...
        st.markdown('<div class="section-title">Crecimiento del patrimonio neto real 📊</div>', unsafe_allow_html=True)
        st.markdown('<p class="info-box">Este gráfico visualiza la construcción de riqueza: El valor de mercado menos la deuda remanente 📊.</p>', unsafe_allow_html=True)
        ...
        fig_p.update_layout(title="Valorización acumulada vs Saldo deudor", height=450, paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', font=dict(family=SYS_SANS, color="white"))
        st.plotly_chart(fig_p, use_container_width=True)

        st.markdown('<div class="footer-tip">🏔️ <b>Tip financiero:</b> La plusvalía es el componente "invisible" que suele duplicar la rentabilidad total del inversionista inmobiliario en Lima 🇵🇪.</div>', unsafe_allow_html=True)

    # --------------------------------------------------------- PESTAÑA 3
    with tabs[2]:
        st.markdown('<div class="section-title">Matrices de sensibilidad operativa ⚖️</div>', unsafe_allow_html=True)
        st.markdown('<p class="info-box">Evaluación de la resiliencia del proyecto ante fluctuaciones en la ocupación y precios del mercado 🚦.</p>', unsafe_allow_html=True)
        
        def color_roi(val):
            c = '#f85149' if val < 5 else '#d29922' if val < 10 else '#3fb950'
            return f'background-color: {c}; color: #000; font-weight: bold; font-family: {SYS_SANS};'

        # Sensibilidad 1
        st.subheader("Efecto de la ocupación en el ROI")
        st.markdown('<p class="info-box">Sensibilidad basada en días ocupados al mes (intervalos de 5 días) 🌙.</p>', unsafe_allow_html=True)
        ...
        # Sensibilidad 2
        st.subheader("Efecto de la tarifa en el ROI")
        st.markdown('<p class="info-box">Sensibilidad basada en el precio por noche (intervalos de S/. 25) 💰.</p>', unsafe_allow_html=True)
        ...
        st.markdown('<div class="footer-tip">⚠️ <b>Gestión de riesgos:</b> Si el ROI cae a zona roja, se recomienda revisar los costos operativos o mejorar los amenities para justificar una tarifa mayor.</div>', unsafe_allow_html=True)

    # --------------------------------------------------------- PESTAÑA 4
    with tabs[3]:
        st.markdown('<div class="section-title">Veredicto: comparativa tradicional vs airbnb 🔄</div>', unsafe_allow_html=True)
        st.markdown('<p class="info-box">Diferencial de beneficios líquidos entre el modelo de rentas cortas y el alquiler residencial convencional ⚖️.</p>', unsafe_allow_html=True)
        ...
        with cc[3]: st.markdown(f'<div class="card-base bg-indigo"><div class="label-card">Diferencial de retornos 🚀</div><div class="val-pos" style="color:white;">{dif_roi:+.1f}%</div></div>', unsafe_allow_html=True)
        
        st.write("---")
        st.markdown('<p class="info-box"><b>Interpretación del gráfico:</b> Las barras muestran la utilidad neta anualizada. Airbnb maximiza el flujo de caja inmediato a cambio de una mayor gestión operativa 📊.</p>', unsafe_allow_html=True)
        
        labels = ['MODELO AIRBNB', 'ALQUILER TRADICIONAL']
        ...
        fig_c.update_layout(title="Utilidad líquida neta anualizada (S/.)", height=550, paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', font=dict(family=SYS_SANS, color="white"))
        st.plotly_chart(fig_c, use_container_width=True)
        
        st.markdown('<p class="info-box" style="text-align:center;"><b>Análisis de variables:</b> La ventaja de Airbnb compensa el riesgo de vacancia y los gastos operativos más intensos. El modelo tradicional es estable pero su rentabilidad es significativamente inferior en el corto plazo.</p>', unsafe_allow_html=True)

        st.markdown('<div class="footer-tip">🏁 <b>Dictamen final:</b> Para un perfil que busca acelerar la creación de patrimonio, el modelo Airbnb es la opción superior bajo este escenario 🇵🇪.</div>', unsafe_allow_html=True)
...
def generate_master_pdf(d):
    ...
    pdf.cell(0, 25, "AUDITORIA DE INVERSION INMOBILIARIA JM", ln=True, align='C')
    pdf.set_font("Helvetica", '', 10); pdf.cell(0, 5, "CONSULTORIA POR: ING. JANCARLO MENDOZA - EXPERTO INTEGRAL", ln=True, align='C')
    pdf.cell(0, 5, f"LIMA, PERU | EMITIDO: {datetime.now().strftime('%d/%m/%Y %H:%M')}", ln=True, align='C')
    ...
    pdf.multi_cell(0, 5, "Analisis del desembolso liquido requerido para la ejecucion del proyecto. Incluye la cuota inicial (20%) mas la adecuacion de diseño y equipamiento para rentas cortas.")
    pdf.ln(2); rows = [["ITEM", "VALOR (S/.)"], ["Precio Activo Inmobiliario", f"{d['val']:,.0f}"], ["Capital Propio (Equity)", f"{d['ini']:,.0f}"], ["Presupuesto Amoblado", f"{d['inv_a']:,.0f}"], ["DESEMBOLSO TOTAL REQUERIDO", f"{d['inv_t']:,.0f}"]]
    for r in rows:
        pdf.cell(100, 7, r[0], 1); pdf.cell(90, 7, r[1], 1, ln=True, align='R')

    # Bloque 2: Operatividad
    pdf.ln(10); pdf.set_font("Helvetica", 'B', 14); pdf.cell(0, 10, "2. Analisis de rentabilidad operativa", ln=True)
    pdf.set_font("Helvetica", '', 9)
    pdf.multi_cell(0, 5, f"Determinacion de la utilidad neta. Se proyecta un ROI Anual del {d['roi']:.2f}% bajo el modelo Airbnb, con un flujo neto mensual de S/. {d['flujo']:,.2f}. El tiempo estimado de recuperacion es de {d['pb']:.1f} años.")

    # Bloque 3: Conclusiones por pestaña
    pdf.ln(10); pdf.set_font("Helvetica", 'B', 14); pdf.cell(0, 10, "3. Conclusiones y dictamen del caso", ln=True)
    pdf.set_font("Helvetica", '', 9)
    pdf.multi_cell(0, 5, "- PATRIMONIO: La plusvalia acumulada en 20 años representa una ganancia patrimonial masiva.\n- COMPARATIVA: Airbnb genera un excedente de caja anual de S/. "+f"{d['exc']:,.0f}"+" frente al modelo residencial.\n- RIESGOS: La sensibilidad muestra que incluso con una ocupacion del 60%, el proyecto se mantiene solvente.")

    # Bloque 4: Recomendaciones JM
    pdf.ln(10); pdf.set_font("Helvetica", 'B', 14); pdf.cell(0, 10, "4. Recomendaciones estrategicas del consultor", ln=True)
    pdf.set_font("Helvetica", '', 9)
    pdf.multi_cell(0, 5, "1. Aplique precios dinamicos segun eventos locales en Lima para maximizar el ROI.\n2. Invierta en fotografia profesional para aumentar el click-through rate en un 30%.\n3. Automatice la gestion de llaves para reducir el costo operativo mensual.")

    pdf.ln(15); pdf.set_font("Helvetica", 'I', 8)
    pdf.multi_cell(0, 4, "AVISO: Este reporte es un analisis tecnico financiero. Jancarlo Mendoza no garantiza retornos futuros ante cambios externos en la legislacion tributaria o fluctuaciones de mercado inmobiliario.")
...
if st.session_state.authenticated:
    ...
    if st.button("📥 GENERAR INFORME DE AUDITORIA COMPLETO (PDF)"):
        ...
        st.download_button("Descargar Informe Estratégico", data=pdf_bytes, file_name=f"Auditoria_Inmobiliaria_JM.pdf")
