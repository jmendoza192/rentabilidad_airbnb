...
if check_password():
    ...
    with tab1: # CONGELADO (V16)
        ...
        with col_bench: st.markdown(f"""<div class="compare-card"><b>📊 5 Referencias de Mercado (ROI Promedio):</b><br>
        ...
        if año_rec: st.markdown(f"""<div class="highlight-card"><span style="color: #8899a6; font-size: 0.9rem;">TIEMPO ESTIMADO DE RECUPERACIÓN</span><br><span style="color: #3b82f6; font-size: 2.2rem; font-weight: bold;">{año_rec:.1f} Años</span><br><span style="color: #00ffcc; font-size: 0.9rem;">En este punto habrás recuperado los S/. {inversion_total_real:,.0f} invertidos.</span></div>""", unsafe_allow_html=True)
    ...
    with tab2:
        ...
        st.markdown('<p class="section-desc">Nota: Aumento del valor comercial del inmueble por factores de mercado.</p>', unsafe_allow_html=True)
        
        plus_val = st.slider("Plusvalía Anual (%)", 0.0, 10.0, 4.0)
        
        # --- NUEVAS TARJETAS DE VALORIZACIÓN EN SOLES ---
        col_5, col_10, col_15, col_20 = st.columns(4)
        v_5 = (val_depa * (1 + plus_val/100)**5) - val_depa
        v_10 = (val_depa * (1 + plus_val/100)**10) - val_depa
        v_15 = (val_depa * (1 + plus_val/100)**15) - val_depa
        v_20 = (val_depa * (1 + plus_val/100)**20) - val_depa
        
        col_5.metric("Valorización 5 años", f"S/. {v_5:,.0f}")
        col_10.metric("Valorización 10 años", f"S/. {v_10:,.0f}")
        col_15.metric("Valorización 15 años", f"S/. {v_15:,.0f}")
        col_20.metric("Valorización 20 años", f"S/. {v_20:,.0f}")
        st.markdown('<p class="info-text" style="text-align: center;">Dinero adicional generado por el simple paso del tiempo y demanda de la zona.</p>', unsafe_allow_html=True)
        
        # Lógica de Gráfico (V16)
        ...
        fig_p = go.Figure(); fig_p.add_trace(go.Bar(x=años_p, y=v_mkt, name="Valor Propiedad", marker_color='#1f2630')); fig_p.add_trace(go.Scatter(x=años_p, y=eq, name="Equity (Patrimonio)", fill='tozeroy', line=dict(color='#00ffcc', width=3)))
        fig_p.update_layout(height=400, barmode='overlay', paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', font_color="white", title="Patrimonio vs Deuda (Intersección = Propiedad Pagada)")
        st.plotly_chart(fig_p, use_container_width=True)
        st.markdown('<p class="info-text">💡 El <b>Equity</b> crece conforme pagas la deuda y el mercado revaloriza la zona.</p>', unsafe_allow_html=True)

    with tab3: # CONGELADO (V16)
