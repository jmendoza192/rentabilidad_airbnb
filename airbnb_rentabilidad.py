...
if check_password():
    ...
    with tab1:
        ...
        st.markdown('<div class="info-text">Capital líquido necesario: 20% inicial bancaria + presupuesto de amoblado y equipamiento inicial.</div>', unsafe_allow_html=True)
        ...
        df_flujo = pd.DataFrame({
            ...
            "Detalle": ["Neto plataforma.", "Crédito hipotecario.", "Mantenimiento/Servicios.", "Impuesto 5%.", "Utilidad líq."]
        })
        st.table(df_flujo)
        
        st.markdown('<div class="section-title">Proyección de Payback (Retorno de Inversión)</div>', unsafe_allow_html=True)
        ...
        if año_rec: 
            ...
            st.markdown(f"""<div class="info-text"><b>Análisis del Punto de Recuperación:</b> Este valor representa el tiempo estimado en que los flujos netos acumulados igualan su desembolso inicial de S/. {inversion_total_real:,.0f}. Indica el fin de la etapa de riesgo de capital y el inicio de la fase de acumulación de riqueza neta sobre la inversión original.</div>""", unsafe_allow_html=True)
        ...
        fig_pb.update_layout(height=600, paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', font_color="white", xaxis_title="Años", yaxis_title="S/. Acumulados")
    ...
    with tab2:
        ...
        st.markdown('<div class="audit-note"><b>📌 Nota de Auditoría:</b> La apreciación del activo suele ser el componente más fuerte de la rentabilidad patrimonial.</div>', unsafe_allow_html=True)

    # --- TAB 3: SENSIBILIDAD (CORREGIDO) ---
    with tab3:
        st.markdown('<div class="section-title">Análisis de Resiliencia y Estrés</div>', unsafe_allow_html=True)
        
        # TARJETAS MÉTRICAS EXPLICATIVAS
        ...
        st.markdown('<div class="info-text"><b>Métricas Críticas:</b> El punto de equilibrio indica las noches mínimas para no perder dinero. Cualquier noche por encima de este valor es utilidad neta.</div>', unsafe_allow_html=True)
        ...
        with c_o2:
            ...
            fig_o.update_layout(height=400, paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', font_color="white", xaxis_title="Noches/Mes", yaxis_title="ROI %")
        ...
        with c_t2:
            ...
            fig_t.update_layout(height=400, paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', font_color="white", xaxis_title="Tarifa S/.", yaxis_title="ROI %")
            st.plotly_chart(fig_t, use_container_width=True)
        
        st.markdown('<div class="audit-note"><b>📌 Nota de Riesgo:</b> Una caída del 15% en la ocupación tiene un impacto mayor en el ROI que una caída del 10% en la tarifa diaria.</div>', unsafe_allow_html=True)
    ...
    with tab4:
        ...
        st.markdown('<div class="audit-note"><b>📌 Conclusión:</b> La renta corta maximiza el flujo de caja, mientras que la tradicional minimiza el riesgo operativo.</div>', unsafe_allow_html=True)
