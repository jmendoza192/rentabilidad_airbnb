...
# --- 1. CONFIGURACIÓN Y SEGURIDAD (V37 INTACTA) ---
try:
...
# --- 2. FUNCIÓN GENERADORA DE PDF (ÚNICO LUGAR DE MEJORAS) ---
def generate_pdf(d):
    pdf = FPDF()
    pdf.add_page()
    ...
    pdf.cell(200, 10, "INFORME TECNICO DE AUDITORIA INMOBILIARIA", ln=True, align='C')
    pdf.set_font("Arial", 'I', 10)
    pdf.cell(200, 8, f"Consultor: Ing. Jancarlo Mendoza - Experto Integral Inmobiliario", ln=True, align='C')
    pdf.cell(200, 8, f"Fecha: {datetime.now().strftime('%d/%m/%Y %H:%M')}", ln=True, align='C')
    pdf.line(10, 38, 200, 38)
    pdf.ln(12)

    # SECCIÓN 1: FLUJOS (Data Pestaña 1)
    pdf.set_font("Arial", 'B', 12)
    pdf.cell(0, 10, "1. ESTRUCTURA DE FLUJOS Y RETORNO", ln=True)
    pdf.set_font("Arial", '', 10)
    pdf.multi_cell(0, 6, f"- Inversion Total (Capex): S/. {d['inv_total']:,.2f}\n"
                         f"- Ingreso Bruto Estimado: S/. {d['ing_bruto']:,.2f}\n"
                         f"- Cuota Hipotecaria: S/. {d['cuota']:,.2f}\n"
                         f"- Utilidad Liquida Mensual: S/. {d['flujo_neto']:,.2f}\n"
                         f"- Tiempo de Recuperacion: {d['payback']:.1f} años.")
    pdf.set_text_color(80, 80, 80)
    pdf.multi_cell(0, 5, "NOTA TECNICA: El flujo neto se calcula descontando la cuota bancaria, mantenimiento y el impuesto a la renta de primera categoria (5%).")
    pdf.set_text_color(0, 0, 0); pdf.ln(5)

    # SECCIÓN 2: PLUSVALIA (Data Pestaña 2)
    pdf.set_font("Arial", 'B', 12)
    pdf.cell(0, 10, "2. VALORIZACION Y PATRIMONIO (EQUITY)", ln=True)
    pdf.set_font("Arial", '', 10)
    pdf.multi_cell(0, 6, f"- Plusvalia Acumulada (10 años): S/. {d['plus_10']:,.2f}\n"
                         f"- Plusvalia Acumulada (20 años): S/. {d['plus_20']:,.2f}\n"
                         f"- Valor Final Proyectado del Activo: S/. {d['val_orig'] + d['plus_20']:,.2f}")
    pdf.multi_cell(0, 5, "TIP: La plusvalia es capital ganado que se liquida al vender. Durante el prestamo, usted construye Equity mientras el inquilino (Airbnb) paga la deuda.")
    pdf.ln(5)

    # SECCIÓN 3: SENSIBILIDAD (Data Pestaña 3)
    pdf.set_font("Arial", 'B', 12)
    pdf.cell(0, 10, "3. SENSIBILIDAD Y ANALISIS DE RIESGO", ln=True)
    pdf.set_font("Arial", '', 10)
    pdf.multi_cell(0, 6, f"- Punto de Equilibrio Operativo: {d['be_days']} dias/mes.\n"
                         f"- ROI Anual Proyectado: {d['roi']:.2f}%")
    pdf.set_text_color(200, 0, 0)
    pdf.multi_cell(0, 5, "ALERTA: Si la ocupacion es menor al punto de equilibrio, el inmueble requiere inyeccion de capital externo para cubrir la cuota bancaria.")
    pdf.set_text_color(0, 0, 0); pdf.ln(5)

    # SECCIÓN 4: COMPARATIVA (Data Pestaña 4)
    pdf.set_font("Arial", 'B', 12)
    pdf.cell(0, 10, "4. COMPARATIVA DE MODELOS DE RENTA", ln=True)
    pdf.set_font("Arial", '', 10)
    pdf.multi_cell(0, 6, f"- Diferencial Anual Airbnb vs Tradicional: S/. {d['ventaja']:,.2f}\n"
                         f"- Factor de Rentabilidad: {d['factor']:.1f}x veces mas eficiente.")
    pdf.set_font("Arial", 'B', 10)
    pdf.multi_cell(0, 5, "CONCLUSION: La gestion en Airbnb permite una optimizacion del flujo de caja superior a la renta tradicional, mitigando el impacto de las tasas de interes actuales.")
