...
# =========================================================
# 0. CONFIGURACIÓN Y ESTÉTICA SANS-SERIF PURA
# =========================================================
try:
...
# Pila de fuentes de sistema (Sin dependencias externas)
...
if check_password():
    st.markdown(f"""
        <style>
        /* Eliminación total de fuentes externas y aplicación de Sans-Serif de sistema */
        ...
        /* Ocultar botones +/- en selectores numéricos */
    ...
    with st.sidebar:
        st.header("Parámetros de auditoría ⚙️")
        
        with st.expander("🏠 Activo e inversión", expanded=True):
            val_depa = st.number_input("Precio del inmueble (S/.)", value=485000)
            inv_amoblado = st.number_input("Inversión amoblado (S/.)", value=58000)
            
        with st.expander("🏦 Financiamento bancario", expanded=False):
            tcea = st.number_input("Tcea bancaria (%)", value=9.5)
            plazo_años = st.selectbox("Plazo del crédito (Años)", [10, 15, 20, 25], index=2)
            
        with st.expander("🏨 Operación Airbnb", expanded=False):
            tarifa_base = st.number_input("Tarifa por noche (S/.)", value=390)
            ocupacion_act = st.selectbox("Días ocupados al mes", list(range(1, 31)), index=19)
            
        with st.expander("🏠 Modelo tradicional", expanded=False):
            renta_trad = st.number_input("Renta mensual (S/.)", value=3300)
    ...
    with tabs[0]:
        ...
        st.markdown('<p class="info-box">Cálculo del capital propio y el apalancamiento necesario para la puesta en marcha del activo inmobiliario 🏢.</p>', unsafe_allow_html=True)
        ...
        st.markdown('<p class="info-box">Determinación del flujo de caja libre tras cubrir costos de deuda y mantenimiento operativo 🧾.</p>', unsafe_allow_html=True)
        ...
        with c5: st.markdown(f'<div class="card-base bg-red-dark"><div class="label-card">Cuota hipoteca 🏦</div><div class="val-neg">S/. -{cuota:,.0f}</div></div>', unsafe_allow_html=True)
        with c6: st.markdown(f'<div class="card-base bg-red-dark"><div class="label-card">Gastos operativos 🧾</div><div class="val-neg">S/. -{gastos_op:,.0f}</div></div>', unsafe_allow_html=True)
        ...
        with cr2:
            st.table(pd.DataFrame({
                "Activo": ["ROI AIRBNB", "S&P 500", "Factoring", "Cajas Municipales", "Bonos del Tesoro", "Plazo Fijo"],
                "Retorno %": [f"{roi_airbnb:.1f}%", "10.0%", "12.5%", "8.2%", "5.6%", "7.0%"]
            }))
        ...
        with cp1:
            ...
            st.markdown('<p class="info-box">Periodo estimado para que las utilidades acumuladas devuelvan el 100% de la inversión líquida inicial 🏛️.</p>', unsafe_allow_html=True)
        with cp2:
            fig_pb = go.Figure()
            ...
            fig_pb.update_layout(title="Curva de balance patrimonial acumulado", height=350, paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', font=dict(family=SYS_SANS, color="white"))
            st.plotly_chart(fig_pb, use_container_width=True)

        st.markdown('<div class="footer-tip">💡 <b>Tip financiero:</b> Un flujo neto positivo durante el periodo hipotecario significa que el activo se paga solo y genera excedentes inmediatos 📊.</div>', unsafe_allow_html=True)

    # --------------------------------------------------------- PESTAÑA 2
    with tabs[1]:
        st.markdown('<div class="section-title">Evolución del patrimonio y plusvalía 🏔️</div>', unsafe_allow_html=True)
        st.markdown('<p class="info-box">Proyección del incremento del valor de mercado del activo inmobiliario en horizontes de largo plazo 📈.</p>', unsafe_allow_html=True)
        p_slider = st.slider("Plusvalía anual (%)", 0.0, 10.0, 4.0)
        ...
        st.markdown('<p class="info-box"><b>Nota informativa:</b> Estos montos representan la ganancia de capital bruta por valorización del inmueble 🏔️.</p>', unsafe_allow_html=True)
        
        st.markdown('<div class="section-title">Activo total vs deuda bancaria 📊</div>', unsafe_allow_html=True)
        ...
        fig_p.update_layout(title="Valorización patrimonial vs Saldo hipotecario", height=450, paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', font=dict(family=SYS_SANS, color="white"))
        st.plotly_chart(fig_p, use_container_width=True)

        st.markdown('<div class="footer-tip">📊 <b>Recomendación patrimonial:</b> El equity es su riqueza real. Al año 15, la mayor parte de su propiedad ya es capital propio libre de deuda.</div>', unsafe_allow_html=True)

    # --------------------------------------------------------- PESTAÑA 3
    with tabs[2]:
        st.markdown('<div class="section-title">Análisis de sensibilidad operativa ⚖️</div>', unsafe_allow_html=True)
        st.markdown('<p class="info-box">Identificación de puntos de equilibrio y escenarios de estrés financiero 🚦.</p>', unsafe_allow_html=True)
        ...
        st.markdown('<p class="info-box">Muestra el retorno de inversión según los días de reserva mensuales (Cada 5 días) 🌙.</p>', unsafe_allow_html=True)
        ...
        with cs1_g:
            ...
            fig_d.update_layout(height=650, title="Curva de rentabilidad por ocupación", xaxis_title="Días ocupados", yaxis_title="ROI %", paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', font=dict(family=SYS_SANS, color="white"))
            st.plotly_chart(fig_d, use_container_width=True)

        # Sensibilidad 2
        st.subheader("Sensibilidad: Tarifa noche vs ROI")
        st.markdown('<p class="info-box">Muestra el retorno de inversión según el precio por noche (Cada S/. 25) 💰.</p>', unsafe_allow_html=True)
        ...
        with cs2_g:
            ...
            fig_t.update_layout(height=650, title="Curva de rentabilidad por precio", xaxis_title="Precio S/.", yaxis_title="ROI %", paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', font=dict(family=SYS_SANS, color="white"))
            st.plotly_chart(fig_t, use_container_width=True)

        st.markdown('<div class="footer-tip">⚠️ <b>Dictamen de riesgo:</b> Un ROI en rojo sugiere que el activo no cubre el costo de oportunidad. Considere mejoras en el interiorismo para subir de tramo tarifario 🚦.</div>', unsafe_allow_html=True)

    # --------------------------------------------------------- PESTAÑA 4
    with tabs[3]:
        st.markdown('<div class="section-title">Comparativa tradicional vs airbnb 🔄</div>', unsafe_allow_html=True)
        st.markdown('<p class="info-box">Evaluación del excedente financiero entre el modelo de corta estancia y el alquiler residencial convencional ⚖️.</p>', unsafe_allow_html=True)
        ...
        st.write("---")
        st.markdown('<p class="info-box"><b>Análisis visual:</b> El gráfico compara la utilidad neta anualizada (caja libre). Airbnb maximiza el flujo a costa de una mayor gestión operativa 📊.</p>', unsafe_allow_html=True)
        ...
        fig_c.update_layout(title="Utilidad neta anualizada proyectada", height=550, paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', font=dict(family=SYS_SANS, color="white"))
        st.plotly_chart(fig_c, use_container_width=True)
        
        st.markdown('<p class="info-box" style="text-align:center;"><b>Explicación operativa:</b> La brecha de rentabilidad justifica la inversión en gestión y mantenimiento intensivo del modelo Airbnb 📊.</p>', unsafe_allow_html=True)

        st.markdown('<div class="footer-tip">🏁 <b>Conclusión:</b> El modelo Airbnb es el vehículo óptimo para la capitalización acelerada en el mercado de Lima 🇵🇪.</div>', unsafe_allow_html=True)

# =========================================================
# LÓGICA DE CIERRE Y EXPORTACIÓN
# =========================================================
if st.session_state.authenticated:
    ...
    if not st.session_state.assessment_finished:
        ...
        def generate_master_pdf(d):
            ...
            pdf.set_font("Helvetica", '', 9); pdf.cell(0, 5, f"CODIGO: JM-{code} | EMITIDO: {datetime.now().strftime('%d/%m/%Y')}", ln=True, align='C')
            
            pdf.ln(35); pdf.set_text_color(0, 0, 0)
            # Secciones
            pdf.set_font("Helvetica", 'B', 11); pdf.cell(0, 10, "1. Estructura de capital inicial", ln=True)
            pdf.set_font("Helvetica", '', 9); pdf.multi_cell(0, 5, "Analisis del desembolso liquido necesario. Se incluye cuota inicial (20%) y presupuesto de equipamiento premium.")
            pdf.ln(2); rows = [["Detalle", "Monto (S/.)"], ["Valor Inmueble", f"{d['val']:,.0f}"], ["Cuota Inicial", f"{d['ini']:,.0f}"], ["Inversion Amoblado", f"{d['inv_a']:,.0f}"], ["CAPITAL TOTAL", f"{d['inv_t']:,.0f}"]]
            for r in rows:
                pdf.cell(90, 7, r[0], 1); pdf.cell(90, 7, r[1], 1, ln=True, align='R')
            
            pdf.ln(8); pdf.set_font("Helvetica", 'B', 11); pdf.cell(0, 10, "2. Rentabilidad y Cashflow", ln=True)
            pdf.set_font("Helvetica", '', 9); pdf.multi_cell(0, 5, f"ROI Proyectado: {d['roi']:.2f}%. Flujo neto mensual: S/. {d['flujo']:,.2f}. Recuperacion del capital en {d['pb']:.1f} años.")
            
            pdf.ln(8); pdf.set_font("Helvetica", 'B', 11); pdf.cell(0, 10, "3. Conclusiones y recomendaciones", ln=True)
            pdf.multi_cell(0, 5, "1. Optimice la gestion de precios dinamicos.\n2. Mantenga un estandar de diseño 'instagrammable' para elevar tarifa.\n3. Reinvierta excedentes en amortizacion de capital para reducir intereses.")
            
            pdf.ln(12); pdf.set_font("Helvetica", 'I', 7)
            pdf.multi_cell(0, 4, "AVISO: Este es un modelo de estudio tecnico-financiero. No garantiza rentabilidades reales ante cambios en leyes, impuestos o mercado inmobiliario peruano.")
            return pdf.output(dest='S').encode('latin-1'), f"Auditoria_Inmobiliaria_JM_{code}.pdf"

        p_bytes, p_name = generate_master_pdf({
            "val": val_depa, "ini": inicial, "inv_a": inv_amoblado, "inv_t": inv_total,
            "roi": roi_airbnb, "flujo": flujo_neto, "pb": rec, "exc": ventaja_anual
        })
        st.download_button("📥 EXPORTAR REPORTE DE AUDITORIA (PDF)", data=p_bytes, file_name=p_name)
